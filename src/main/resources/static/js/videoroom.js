					...
					janus.attach({
						...
						onlocalstream: function(stream) {
							...
							if ($('#myvideo').length === 0) {
								...
								$('#cam-off').click(toggleVideo);
								$('#cam-on').click(toggleVideo);
							}
...
function registerUsername() {
	...
	if ($('#roomname').length === 0) {
		...
		sfutest.send({
			message: createRoom, success: function(result) {
...
function participantsList(room) {
	...
	sfutest.send({
		"message": roomPQuery, success: function(result) {
