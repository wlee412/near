...
function isPhoneUnique(phone) {
    ...
    // GET /mypage/check-phone?phone=01012345678
    xhr.open('GET', `/client/check-phone?phone=${encodeURIComponent(phone)}`, false);
    xhr.send();
    if (xhr.status === 200) {
...
function validateUpdateForm() {
	...
	// ✅ 이름 유효성
	const nameRegex = /^[가-힣]+$|^[a-zA-Z]+( [a-zA-Z]+)*$/;
	if (!name || !nameRegex.test(name)) {
		console.log("✅ name: ", name);
		console.log("✅ nameRegex test result: ", nameRegex.test(name));
