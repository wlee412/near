...
<body>
			...
			<div class="mypage-body">
				<aside class="mypage-sidebar">
					<a
						...
						class="sidebar-button active">정보수정</a> <a
				...
				<section class="main-section" id="contentArea">
					...
					<form id="updateForm" action="/mypage/update" method="post"
						...
						<div class="form-group">
							<label>아이디</label> <input type="text" name="clientId" class="input-md"
								value="${client.clientId}" readonly>
						</div>

						<!-- 이름 -->
						<div class="form-group">
							<label>이름</label> <input type="text" id="name" name="name" class="input-md"
								value="${client.name}">
						...
						<!-- 휴대폰 -->
						<div class="form-group">
							<label>휴대폰 번호</label> <input type="tel" id="phone" name="phone" class="input-md"
						...
						<div class="form-group">
							<label>주소</label>
							<div class="input-row">
								<input type="text" name="zipcode" id="postcode" class="input-sm"
									placeholder="우편번호" readonly style="width: 150px;"
									value="${client.zipcode}" />
								<button type="button" onclick="execDaumPostcode()" class="btn">주소 찾기</button>
							</div>
							<input type="text" id="roadAddress" class="input-lg" placeholder="도로명 주소" 
								name="addrBase" readonly required style="margin-top: 8px;"
								value="${client.addrBase}" /> <input type="text" class="input-lg"
								id="detailAddress" placeholder="상세 주소 입력" name="addrDetail"
								required style="margin-top: 8px;" value="${client.addrDetail}" />
						</div>
						...
						<!-- 생년월일 -->
						<div class="form-group">
							<label>생년월일</label> <input type="date" name="birth" id="birth" class="input-md"
								value="${client.birth}">
						</div>
						...
						<c:choose>
								...
								<div class="form-group">
									...
									<div class="radio-group">
										<label> <input type="radio" name="gender" value="M"
											<c:if test="${client.gender eq 'M'}">checked</c:if>>
											남자
										</label> <label> <input type="radio" name="gender" value="F"
											<c:if test="${client.gender eq 'F'}">checked</c:if>>
											여자
										</label>
									</div>
								</div>
							...
							<c:otherwise>
								<div class="form-group">
									...
									<div class="radio-group">
										<label> <input type="radio" name="gender" value="M"
											<c:if test="${client.gender eq 'M'}">checked</c:if> disabled>
											남자
										</label> <label> <input type="radio" name="gender" value="F"
											<c:if test="${client.gender eq 'F'}">checked</c:if> disabled>
											여자
										</label>
									</div>
								</div>
						...
						<div class="form-group">
							...
							<div class="checkbox-group">
								<label> <input type="checkbox" name="interestList"
									value="우울감"
									<c:if test="${interestList.contains('우울감')}">checked</c:if> />우울감
								</label> <label> <input type="checkbox" name="interestList"
									value="불안"
									<c:if test="${interestList.contains('불안')}">checked</c:if> />불안
								</label> <label> <input type="checkbox" name="interestList"
									value="자존감"
									<c:if test="${interestList.contains('자존감')}">checked</c:if> />자존감
								</label> <label> <input type="checkbox" name="interestList"
									value="수면 문제"
									<c:if test="${interestList.contains('수면 문제')}">checked</c:if> />수면
									문제
								</label> <label> <input type="checkbox" name="interestList"
									value="스트레스"
									<c:if test="${interestList.contains('스트레스')}">checked</c:if> />스트레스
								</label> <label> <input type="checkbox" name="interestList"
									value="공황/불안발작"
									<c:if test="${interestList.contains('공황/불안발작')}">checked</c:if> />공황/불안발작
								</label> <label> <input type="checkbox" name="interestList"
									value="대인관계"
									<c:if test="${interestList.contains('대인관계')}">checked</c:if> />대인관계
								</label> <label> <input type="checkbox" name="interestList"
									value="섭식 문제"
									<c:if test="${interestList.contains('섭식 문제')}">checked</c:if> />섭식
									문제
								</label> <label> <input type="checkbox" name="interestList"
									value="ADHD"
									<c:if test="${interestList.contains('ADHD')}">checked</c:if> />ADHD
								</label> <label> <input type="checkbox" name="interestList"
									value="자기통제"
									<c:if test="${interestList.contains('자기통제')}">checked</c:if> />자기통제
								</label>
							</div>
						</div>
						...
						<div class="form-group checkbox-group">
							<label> <input type="checkbox" name="agreeUpdate"
								required> 정보 수정 동의 (필수)
							</label>
						</div>
						...
						<!-- 버튼 -->
						  <div class="button-row">
						    ...
						    <button type="button" class="btn btn-cancel"
						      onclick="location.href='/mypage/mypageProfile'">취소</button>
  						  </div>
					</form>
				</section>
				</div>
	...
	<script src="${pageContext.request.contextPath}/js/mypageUpdate.js"
		defer></script>
