					...
					<div class="form-group">
						...
						<div class="checkbox-group">
							<label><input type="checkbox" name="interestList"
								value="우울감"
								<c:if test="${interestList.contains('우울감')}">checked</c:if> />우울감</label>
							<label><input type="checkbox" name="interestList"
								value="불안"
								<c:if test="${interestList.contains('불안')}">checked</c:if> />불안</label>
							<label><input type="checkbox" name="interestList"
								value="자존감"
								<c:if test="${interestList.contains('자존감')}">checked</c:if> />자존감</label>
							<label><input type="checkbox" name="interestList"
								value="수면 문제"
								<c:if test="${interestList.contains('수면 문제')}">checked</c:if> />수면
								문제</label> <label><input type="checkbox" name="interestList"
								value="스트레스"
								<c:if test="${interestList.contains('스트레스')}">checked</c:if> />스트레스</label>
							<label><input type="checkbox" name="interestList"
								value="공황/불안발작"
								<c:if test="${interestList.contains('공황/불안발작')}">checked</c:if> />공황/불안발작</label>
							<label><input type="checkbox" name="interestList"
								value="대인관계"
								<c:if test="${interestList.contains('대인관계')}">checked</c:if> />대인관계</label>
							<label><input type="checkbox" name="interestList"
								value="섭식 문제"
								<c:if test="${interestList.contains('섭식 문제')}">checked</c:if> />섭식
								문제</label> <label><input type="checkbox" name="interestList"
								value="ADHD"
								<c:if test="${interestList.contains('ADHD')}">checked</c:if> />ADHD</label>
							<label><input type="checkbox" name="interestList"
								value="자기통제"
								<c:if test="${interestList.contains('자기통제')}">checked</c:if> />자기통제</label>
						</div>
					</div>
