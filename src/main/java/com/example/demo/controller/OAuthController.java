...
//        Client client = clientMapper.findBySocialIdAndPlatform(socialId, platform);
		...
		// ★ client가 null이면, 처음 로그인하는 사용자니까 회원 가입 페이지로 보내거나
		// CustomOAuth2UserService 에서 이미 삽입했다면 세션에서 꺼내도록 수정해야 합니다.
		if (client == null) {
			Map<String, Object> attrs = oAuth2User.getAttributes();
			redirectAttributes.addFlashAttribute("clientId", socialId);
			redirectAttributes.addFlashAttribute("name", attrs.get("name"));
			redirectAttributes.addFlashAttribute("email", attrs.get("email"));
			return "redirect:/client/register";
		}

		// 3. 프로필 정보가 부족하면 업데이트 페이지로
		if (client.getBirth().equals(Date.valueOf("1900-01-01")) || client.getGender().equals("N")
				|| client.getAddrBase().isBlank() || client.getAddrDetail().isBlank() || client.getZipcode().isEmpty()
				|| client.getInterest().isBlank()) {
